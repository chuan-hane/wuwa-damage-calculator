...
function initialRenderCompletes() {
  ...
  assert(html.includes('class="stage-language"') && html.includes('class="stage-language-icon"') && html.includes('class="stage-language-select" data-act="language"') && html.includes('value="zh-CN"') && html.includes('value="en-US"') && html.includes('value="ko"') && html.includes('value="ja-JP"'), "topbar should render an icon language dropdown");
  assert(html.includes('class="stage-github-link"') && html.includes('class="stage-github-icon"') && html.includes("https://github.com/chuan-hane/wuwa-damage-calculator"), "topbar should link to the GitHub repository with its mark");
...
function englishRenderCompletes() {
  ...
  assert(html.includes('value="zh-CN"') && html.includes('value="en-US"') && html.includes('value="ko"') && html.includes('value="ja-JP"'), "all language options should remain available");
  __T.state.resultMode = "offset";
  __T.render();
...
function topbarSticksToViewportTop() {
  ...
  assert(css.includes("--topbar-base-h: 62px;") && css.includes("--topbar-h: var(--topbar-base-h);"), "stage shell should reserve enough space for the fixed topbar base row");
  ...
  assert(css.includes(".topbar-damage-summary {\n  min-height: 42px;\n  display: flex;\n  align-items: center;") && /\.topbar-output-slot\s*\{[^}]*height:\s*42px;[^}]*min-height:\s*42px;[^}]*display:\s*flex;[^}]*align-items:\s*center;/s.test(css), "fixed damage and output summaries should share one compact aligned row");
  assert(/\.topbar-damage-summary b\s*\{[^}]*font-size:\s*28px;/s.test(css) && /\.topbar-output-avatar\s*\{[^}]*width:\s*28px;[^}]*height:\s*28px;/s.test(css) && /\.topbar-output-identity b\s*\{[^}]*font-size:\s*var\(--font-md\);/s.test(css), "fixed damage value and compact character summaries should remain readable while scrolling");
  ...
  assert(/\.topbar-output-identity\s*\{[^}]*flex:\s*0 0 auto;/s.test(css) && /\.topbar-output-gear\s*\{[^}]*flex:\s*1 1 auto;/s.test(css), "fixed output slots should preserve character identity before truncating gear details");
  assert(/\.stage-github-link\s*\{[^}]*height:\s*42px;/s.test(css) && /\.stage-language\s*\{[^}]*height:\s*42px;/s.test(css), "topbar actions and fixed output slots should share the same height");
