...
After cloning the repository, open `index.html` directly:

```bash
open index.html
```

You can also host the project with any static file server. There is no build step.

## Verification

```bash
find src data/core data/languages scripts -name '*.js' -print0 | xargs -0 -n 80 node --check
node scripts/verify.js
```

`scripts/verify.js` is the lightweight regression entry point. It covers Resonator mounting, schema references, automatic buffs, Effect and Off-Tune calculations, rounding, and key Resonator regressions.

After changing icon files, regenerate the local icon index:
...
```text
index.html          # Page shell: registers window.WUWA and loads data, icon index, and src/*.js in order
styles.css          # Workbench layout, formula cards, Resonator panel, equipment, and buff styles
src/
  rules.js          # Damage zones, field mappings, skill-level ratios, and other rule constants
  render-helpers.js # Number formatting, HTML escaping, skill formula display, and buff text helpers
  equipment.js      # Weapon, Sonata Effect, lead Echo, and Echo stat normalization
  settlement.js     # Skill parsing, state prerequisites, buff checks, zone aggregation, and damage calculation
  buff-view.js      # Buff cards, grouping, automatic application, and inactive-buff rendering
  panel-view.js     # Resonator panel breakdown, Echo stat inputs, and panel repainting
  stage-view.js     # Top damage stage, skill controls, team slots, and equipment area rendering
  app.js            # Page state, panel breakdown, rendering, and event binding
data/
  icons.js          # Local icon override table generated by scripts/sync-icons.js
  core/             # Language-neutral values, gates, ids, and structured rules
    weapons.js      # Weapon base stats and effect structure
    sonatas.js      # Sonata Effects, lead Echoes, and effect structure
    chara/          # Core Resonator data, one file per Resonator, grouped by debut version
      v1.0/ ... v3.4/
  languages/
    zh-CN/          # Chinese display text: Resonators, weapons, Echoes, UI, terms
    en-US/          # English display text: Resonators, weapons, Echoes, UI, terms
assets/
  icons/
    characters/     # Resonator portrait icons, with filenames matching Resonator ids
    weapons/        # Weapon icons, using official English slugs as filenames
    sonatas/        # Sonata Effect icons, using official English slugs as filenames
    elements/       # Element icons
    missing-icons.json # Local missing-icon report
scripts/
  sync-icons.js     # Scans local icons and generates data/icons.js / missing-icons.json
  verify.js         # Lightweight regression checks
.codex/
  agents/
    wuwa-char-entry.toml # Resonator data-entry workflow notes
```

Core Resonator files keep the "one Resonator, one file" rule and live at `data/core/chara/v<debut-version>/<id>.js`. Display text lives in `data/languages/<language>/chara/v<debut-version>/<id>.js`; the runtime loads core data first, then overlays fields such as `name`, `label`, `desc`, and `excerpt` from the active language pack.
...
The Off-Tune system does not use the ATK/HP/DEF panel, Crit. Rate, Crit. DMG, Attribute DMG Bonus, or Type DMG Bonus.

Formula display rule: normal damage, Effect DMG, and Off-Tune formulas use the same formula-card layout. Cards only show values and breakdowns internally; multiplication signs are rendered between cards by the outer layout.
